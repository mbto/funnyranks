    ...
    public void startConsumeAsync(Partition partition) {
        ...
        for (; ; ) {
            ...
            if (message.getBrokerEvent() != null) {
                ...
                if(debugEnabled) {
                    log.debug(partition + " taked brokerEvent: " + brokerEvent);
                }
                if (brokerEvent == APPLY_CHANGES) {
                    CyclicBarrier cb = (CyclicBarrier) message.getPojo();
                    if (cb.isBroken()) {
                        if(debugEnabled) {
                            log.debug(partition + " synchronization canceled");
                        }
                        continue;
                    }
                    if(debugEnabled) {
                        log.debug(partition + " ready synchronization");
                    }
                    try {
                        cb.await();
                    } catch (Throwable e) {
                        if(debugEnabled) {
                            log.debug(partition + " exception while await synchronization", e);
                        }
                        continue;
                    }
                    if(debugEnabled) {
                        log.debug(partition + " end synchronization");
                    }
