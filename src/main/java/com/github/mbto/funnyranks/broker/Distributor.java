...
public class Distributor {
    ...
    private void consumeSynchronizationEvent(Message<?> originalMessage) {
        ...
        if(isDebugEnabled) {
            log.debug("Started synchronization by brokerEvent " + originalMessage.getBrokerEvent());
        }

        Collection<Partition> partitions = partitionById.values();
        CyclicBarrier cb = new CyclicBarrier(partitions.size() + 1,
            () -> {
                BrokerEvent brokerEvent = originalMessage.getBrokerEvent();
                if (brokerEvent == APPLY_CHANGES) {
                    applyChanges((UInteger) originalMessage.getPojo());
                }
            }
        );
        ...
        if(isDebugEnabled) {
            log.debug("Waiting synchronization");
        }
        try {
    ...
    public void applyChanges(UInteger projectId) {
