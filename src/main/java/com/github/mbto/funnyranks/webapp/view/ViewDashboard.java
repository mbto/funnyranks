...
public class ViewDashboard {
        ...
        for (PortData portData : sortedPortData) {
            ...
            try {
                ...
                if(portData.isPortActive()) {
                    log.warn(portValue + " Flush sessions not registered, " + e.getMessage());
                    warnMsgs.add("Flush sessions " + portValue + " not registered, " + e.getMessage());
                } else {
                    log.info(portValue + " Flush sessions not registered, " + e.getMessage() + " (port not active)");
                    infoMsgs.add("Flush sessions " + portValue + " not registered, " + e.getMessage() + " (port not active)");
                }
                continue;
            }
    ...
    public void makeFakes() {
        if (!brokerHolder.isDevEnvironment() || sortedPortData == null) {
            return;
        }
