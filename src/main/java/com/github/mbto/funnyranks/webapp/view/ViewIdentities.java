...
public class ViewIdentities {
    ...
    public void flushIdentities() {
        ...
        try {
            ...
            if(selectedPortData.isPortActive()) {
                log.warn(selectedPort + " Flush identities not registered, " + e.getMessage());
                fc.addMessage("msgs", new FacesMessage(SEVERITY_WARN,
                        "Flush identities " + selectedPort + " not registered", e.getMessage()));
            } else {
                log.info(selectedPort + " Flush identities not registered, " + e.getMessage() + " (port not active)");
                fc.addMessage("msgs", new FacesMessage(SEVERITY_INFO,
                        "Flush identities " + selectedPort + " not registered", e.getMessage() + " (port not active)"));
            }
            return;
        }
