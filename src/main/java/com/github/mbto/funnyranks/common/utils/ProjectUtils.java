...
public class ProjectUtils {
    ...
    public static String extractIp(String ipRaw) {
        ...
        int pos = ipRaw.indexOf(':');
        if(pos > -1) {
            ipRaw = ipRaw.substring(0, pos);
        }
        //noinspection StringOperationCanBeSimplified
        if (ipRaw.toLowerCase().equals("loopback")) {
            return "127.0.0.1";
        } else {
            ...
            if (matcher.matches()) {
                ...
                if(log.isDebugEnabled()) {
                    log.debug("Skipped extractIp from ipRaw=" + ipRaw);
                }
            }
        }
        return null;
    ...
    public static String extractSteamId(String steamIdRaw) {
        if (StringUtils.isBlank(steamIdRaw)) {
            if(log.isDebugEnabled()) {
                log.debug("Skipped extractSteamId from steamIdRaw=" + steamIdRaw);
            }
            return null;
        }
    ...
    public static Long convertSteamId2ToSteamId64(String steamid2) {
        ...
        if (matcher.matches() && matcher.groupCount() == 2) {
            ...
            if (!isSteamId64HasValidRange(steamid64)) {
                if(log.isDebugEnabled()) {
                    log.debug("Skipped convertSteamId2ToSteamId64, due steamid64=" + steamid64 + " has invalid range");
                }
                return null;
            }
            return steamid64;
            ...
            if(log.isDebugEnabled()) {
                log.debug("Skipped convertSteamId2ToSteamId64 steamid2=" + steamid2);
            }
        }
        return null;
    }
    ...
    public static String convertSteamId64ToSteamId2(Long steamid64) {
        if (steamid64 == null || !isSteamId64HasValidRange(steamid64)) {
            if(log.isDebugEnabled()) {
                log.debug("Skipped convertSteamId2ToSteamId64, due steamid64=" + steamid64 + " is null or has invalid range");
            }
            return null;
        }
    ...
    private static boolean isSteamId64HasValidRange(long steamid64) {
                            ...
                            currentSessionStream = Stream.of(session)
                                    .filter(filteredSession -> {
                                        if (!canMakeIdentity(portData, filteredSession)) {
                                            if (logIgnored) {
                                                String logMsg = buildIgnoredMergeTypeMsg(language, mergeType, name, filteredSession);
                    ...
                    Stream<Session> sessionStream = storage.getArchivedSessions()
                            ...
                            .filter(archivedSession -> {
                                ...
                                if (!canMakeIdentity(portData, archivedSession)) {
                                    if (logIgnored) {
                                        logMsg = buildIgnoredMergeTypeMsg(language, mergeType, name, archivedSession);
                                        log.info(portValue + " " + logMsg);
                                        portData.addMessage(logMsg);
                                    }
                                    return false;
    ...
    private static String buildIgnoredMergeTypeMsg(ProjectLanguage projectLanguage,
                                                   ProjectMergeType mergeType,
                                                   String name,
                                                   Session session) {
        return "Ignored session, due " + mergeType.getLiteral() + " required for mergeType='" + mergeType + "'"
                + ": " + session.toString(name, projectLanguage);
    }
    ...
    public static HikariDataSource buildHikariDataSource(Project project) {
        return buildHikariDataSource(project, null, null);
    }

    public static HikariDataSource buildHikariDataSource(Project project, String transactionIsolation) {
        return buildHikariDataSource(project, null, transactionIsolation);
    }

    public static HikariDataSource buildHikariDataSource(Project project, Properties properties, String transactionIsolation) {
...
//        hds.setIdleTimeout(MINUTES.toMillis(59)); // setMinimumIdle == setMaximumPoolSize = comment, others overrided in application.properties
...
//                          .notEqual(DSL.value(newValue).collate(collation));
    ...
    public static long extractFileSize(java.nio.file.Path path) {
...
//    }
    ...
    public static void fillFakes(Map<UShort, PortData> portDataByPort, Map<UShort, Map<String, Storage>> playersViewByPort) {
        ...
        for (Map.Entry<UShort, PortData> entry : portDataByPort.entrySet()) {
            ...
            // session closes with lastTouchDateTime in MessageHandler.java.flushSessions()..onDisconnected(), can't exceed lastTouchDateTime
